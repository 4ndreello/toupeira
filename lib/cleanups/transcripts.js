...
export function collect({ days, home, now, onProgress }) {
  ...
  const items = [...groups.values()].map((g) => ({
    ...
    size: 0,
    safe: false,
    // the age span of the chats on offer, so the list says how stale a project is
    span: `${age(g.last)}-${age(g.first)}d`,
    note: `${g.h.name}: ${g.files.length} chat(s), ${day(g.first)} → ${day(g.last)}, ${age(g.last)}-${age(g.first)}d old`,
