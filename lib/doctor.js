...
function realDocker() {
  try {
    ...
    return null // docker absent or unhappy: doctor still reports the filesystem rows
  }
}
...
// ponytail: this spot list is static — the upgrade path is discovering more from each tool's own config
export function report({ home, runDocker = realDocker }) {
  ...
  const rows = live.map((s) => ({ ...s, size: sizes.get(s.path) })).sort((a, b) => b.size - a.size)
  let docker = null
  try {
    docker = runDocker()
  } catch {
    docker = null // an injected probe may throw; doctor is read-only and never throws
  }
  return { rows, docker }
}
