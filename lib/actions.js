...
//   frees  false when the target is not a path at all (a ref, a tool's own prune), so
...
export const ACTIONS = {
  ...
  'branch-delete': { tree: false, frees: false, run: ({ action }) => git(['branch', '-D', action.branch], action.repo) !== null },
  ...
  command: {
    ...
    run: ({ action }) => {
      try {
        execFileSync(action.cmd[0], action.cmd.slice(1), { stdio: 'ignore', timeout: 5 * 60_000 })
        return true
      } catch {
        return false
      }
